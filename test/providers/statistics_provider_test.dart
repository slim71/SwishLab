...
void main() {
  ...
  group('UserStatisticsState', () {
    test('copyWith updates fields correctly', () {
      const state = UserStatisticsState(items: [], totalCount: 0, hasMore: false, isLoadingMore: false);
    ...
    test('copyWith keeps existing fields if null provided', () {
      final state = UserStatisticsState(items: [createStat('1')], totalCount: 1, hasMore: true, isLoadingMore: true);
      final updated = state.copyWith();
  ...
  group('UserStatisticsNotifier', () {
    ...
    test('fetchMore skips if already loading or no more data or has error', () async {
      ...
      notifier.state = const AsyncValue.data(UserStatisticsState(items: [], totalCount: 0, hasMore: true, isLoadingMore: true));
      await notifier.fetchMore();
      verifyNever(() => mockRepo.getUserStatistics(any(), limit: any(named: 'limit'), offset: any(named: 'offset')));
